    ...
    <section className="relative py-20 md:py-28">
      <div className="container mx-auto max-w-6xl px-4">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <Reveal>
            <div>
              <div className="text-xs font-medium uppercase tracking-[0.2em] text-emerald-400">UI Push</div>
          ...
          <Reveal delay={0.15}>
            <div ref={mockRef} className="relative">
              <div className="absolute -inset-6 rounded-3xl bg-gradient-to-b from-emerald-500/10 to-transparent blur-2xl" />
              <div className="relative rounded-2xl border border-white/12 bg-[#0a0c12]/90 p-5 shadow-2xl backdrop-blur">
                {/* 卡标题：居中四元 */}
                ...
                <div className="mt-3 flex flex-wrap justify-center gap-1.5">
                  {["🕐 民国十三年", "📍 天枢城", "📈 突破率 12%"].map((t) => (
                    <span key={t} className="rounded-md border border-white/15 bg-white/5 px-2 py-0.5 text-[11px] text-white/70">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {["🪙 灵石 12", "⏳ 寿元 78 年", "📖 主修 引气诀"].map((t) => (
                    <span key={t} className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5 text-[11px] text-white/65">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-3 rounded-xl border border-white/10 p-3">
                  <div className="mb-2 text-[10px] uppercase tracking-wider text-white/40">天资</div>
                  <div className="space-y-1.5">
                    {[
                      ...
                      <div key={n} className="flex items-start gap-2">
                        ...
                        <span className="text-[11px] leading-relaxed text-white/45">剑入凡尘如旧友重逢，修行一日千里。</span>
                      </div>
                    ))}
                  </div>
                </div>
